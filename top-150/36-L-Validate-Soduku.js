...
function validateColumn(board) {
    for (let i = 0; i < board[0].length; i++) { // Loop through each column
        const set = new Set();

        for (let j = 0; j < board.length; j++) { // Loop through each row
            let cell = board[j][i]; // Correct access of column
